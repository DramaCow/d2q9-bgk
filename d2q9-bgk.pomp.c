    ...
    for (int jj = 0; jj < params.nx; jj++)
    {
      ...
      if (!obstacles[ii * params.nx + jj])
      {
        // compute local density total
				double local_density = tmp_cells[ii * params.nx + jj].speeds[0] + tmp_cells[ii * params.nx + jj].speeds[1] + tmp_cells[ii * params.nx + jj].speeds[2]
														 + tmp_cells[ii * params.nx + jj].speeds[3] + tmp_cells[ii * params.nx + jj].speeds[4] + tmp_cells[ii * params.nx + jj].speeds[5]
														 + tmp_cells[ii * params.nx + jj].speeds[6] + tmp_cells[ii * params.nx + jj].speeds[7] + tmp_cells[ii * params.nx + jj].speeds[8];
        ...
        double d_equ0 = w0 * local_density * (1.0 - 1.5*u_sq);
        // axis speeds: weight w1
        double d_equ1 = w1 * local_density * (1.0 + 3.0*u1 + 4.5*(u1*u1) - 1.5*u_sq);
        double d_equ2 = w1 * local_density * (1.0 + 3.0*u2 + 4.5*(u2*u2) - 1.5*u_sq);
        double d_equ3 = w1 * local_density * (1.0 + 3.0*u3 + 4.5*(u3*u3) - 1.5*u_sq);
        double d_equ4 = w1 * local_density * (1.0 + 3.0*u4 + 4.5*(u4*u4) - 1.5*u_sq);
        // diagonal speeds: weight w2
        double d_equ5 = w2 * local_density * (1.0 + 3.0*u5 + 4.5*(u5*u5) - 1.5*u_sq);
        double d_equ6 = w2 * local_density * (1.0 + 3.0*u6 + 4.5*(u6*u6) - 1.5*u_sq);
        double d_equ7 = w2 * local_density * (1.0 + 3.0*u7 + 4.5*(u7*u7) - 1.5*u_sq);
        double d_equ8 = w2 * local_density * (1.0 + 3.0*u8 + 4.5*(u8*u8) - 1.5*u_sq);

        // relaxation step
        cells[ii * params.nx + jj].speeds[0] = tmp_cells[ii * params.nx + jj].speeds[0] + params.omega * (d_equ0 - tmp_cells[ii * params.nx + jj].speeds[0]);
				cells[ii * params.nx + jj].speeds[1] = tmp_cells[ii * params.nx + jj].speeds[1] + params.omega * (d_equ1 - tmp_cells[ii * params.nx + jj].speeds[1]);
				cells[ii * params.nx + jj].speeds[2] = tmp_cells[ii * params.nx + jj].speeds[2] + params.omega * (d_equ2 - tmp_cells[ii * params.nx + jj].speeds[2]);
				cells[ii * params.nx + jj].speeds[3] = tmp_cells[ii * params.nx + jj].speeds[3] + params.omega * (d_equ3 - tmp_cells[ii * params.nx + jj].speeds[3]);
				cells[ii * params.nx + jj].speeds[4] = tmp_cells[ii * params.nx + jj].speeds[4] + params.omega * (d_equ4 - tmp_cells[ii * params.nx + jj].speeds[4]);
				cells[ii * params.nx + jj].speeds[5] = tmp_cells[ii * params.nx + jj].speeds[5] + params.omega * (d_equ5 - tmp_cells[ii * params.nx + jj].speeds[5]);
				cells[ii * params.nx + jj].speeds[6] = tmp_cells[ii * params.nx + jj].speeds[6] + params.omega * (d_equ6 - tmp_cells[ii * params.nx + jj].speeds[6]);
				cells[ii * params.nx + jj].speeds[7] = tmp_cells[ii * params.nx + jj].speeds[7] + params.omega * (d_equ7 - tmp_cells[ii * params.nx + jj].speeds[7]);
				cells[ii * params.nx + jj].speeds[8] = tmp_cells[ii * params.nx + jj].speeds[8] + params.omega * (d_equ8 - tmp_cells[ii * params.nx + jj].speeds[8]);
      }
    }
  }
