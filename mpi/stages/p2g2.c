...
int propagate(const t_param params, t_speed* cells, t_speed* tmp_cells)
{
  ...
  #pragma omp parallel for default(none) shared(cells,tmp_cells) schedule(static)
...
float collision(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles)
{
  ...
  ** are in the scratch-space grid */
  #pragma omp parallel for default(none) shared(cells,tmp_cells,obstacles) schedule(static) reduction(+:tot_u,tot_cells) firstprivate(w,u)
  for (int ii = 0; ii < params.ny; ii++)
  {
    for (int jj = 0; jj < params.nx; jj++)
    {
...
/*
        ...
        d_equ[1] = w1 * local_density * (1.0f + 3.0f*u[1] + 4.5f*(u[1]*u[1]) - 1.5f*u_sq);
        d_equ[2] = w1 * local_density * (1.0f + 3.0f*u[2] + 4.5f*(u[2]*u[2]) - 1.5f*u_sq);
        d_equ[3] = w1 * local_density * (1.0f + 3.0f*u[3] + 4.5f*(u[3]*u[3]) - 1.5f*u_sq);
        d_equ[4] = w1 * local_density * (1.0f + 3.0f*u[4] + 4.5f*(u[4]*u[4]) - 1.5f*u_sq);
        // diagonal speeds: weight w2
        d_equ[5] = w2 * local_density * (1.0f + 3.0f*u[5] + 4.5f*(u[5]*u[5]) - 1.5f*u_sq);
        d_equ[6] = w2 * local_density * (1.0f + 3.0f*u[6] + 4.5f*(u[6]*u[6]) - 1.5f*u_sq);
        d_equ[7] = w2 * local_density * (1.0f + 3.0f*u[7] + 4.5f*(u[7]*u[7]) - 1.5f*u_sq);
        d_equ[8] = w2 * local_density * (1.0f + 3.0f*u[8] + 4.5f*(u[8]*u[8]) - 1.5f*u_sq);
*/
        ...
        for (int kk = 0; kk < NSPEEDS; kk++)
        {
          cells[ii * params.nx + jj].speeds[kk] = (1.0f - params.omega)*tmp_cells[ii * params.nx + jj].speeds[kk] + omega_d_equ[kk];
        }
