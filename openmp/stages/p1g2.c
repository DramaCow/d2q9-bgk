...
float collision(const t_param params, t_speed* cellsIn, t_speed* cellsOut, int* obstacles);
...
float collision(const t_param params, t_speed* cellsIn, t_speed* cellsOut, int* obstacles)
{
  ...
  ** are in the scratch-space grid */
  #pragma omp parallel for default(none) shared(cellsIn,cellsOut,obstacles) schedule(static) reduction(+:tot_u,tot_cells) firstprivate(w,u)
  for (int ii = 0; ii < params.ny; ii++)
  {
    for (int jj = 0; jj < params.nx; jj++)
    {
		  ...
		  float tmp_speeds[NSPEEDS];
	 	  tmp_speeds[0] = cellsIn[ii  * params.nx + jj ].speeds[0]; // central cell, no movement
		  tmp_speeds[1] = cellsIn[ii  * params.nx + x_w].speeds[1]; // east
		  tmp_speeds[2] = cellsIn[y_s * params.nx + jj ].speeds[2]; // north
		  tmp_speeds[3] = cellsIn[ii  * params.nx + x_e].speeds[3]; // west
		  tmp_speeds[4] = cellsIn[y_n * params.nx + jj ].speeds[4]; // south
      tmp_speeds[5] = cellsIn[y_s * params.nx + x_w].speeds[5]; // north-east
      tmp_speeds[6] = cellsIn[y_s * params.nx + x_e].speeds[6]; // north-west
			tmp_speeds[7] = cellsIn[y_n * params.nx + x_e].speeds[7]; // south-west
 			tmp_speeds[8] = cellsIn[y_n * params.nx + x_w].speeds[8]; // south-east 
      ...
      if (!obstacles[ii * params.nx + jj])
      {
        ...
        for (int kk = 0; kk < NSPEEDS; kk++)
        {
          cellsOut[ii * params.nx + jj].speeds[kk] = (1.0f - params.omega)*tmp_speeds[kk] + omega_d_equ[kk];
        }
      ...
      else
      {
				...
				// === REBOUND ===
				// ===============
        cellsOut[ii * params.nx + jj].speeds[1] = tmp_speeds[3];
        cellsOut[ii * params.nx + jj].speeds[2] = tmp_speeds[4];
        cellsOut[ii * params.nx + jj].speeds[3] = tmp_speeds[1];
        cellsOut[ii * params.nx + jj].speeds[4] = tmp_speeds[2];
        cellsOut[ii * params.nx + jj].speeds[5] = tmp_speeds[7];
        cellsOut[ii * params.nx + jj].speeds[6] = tmp_speeds[8];
        cellsOut[ii * params.nx + jj].speeds[7] = tmp_speeds[5];
        cellsOut[ii * params.nx + jj].speeds[8] = tmp_speeds[6];
      }
    }
  }
