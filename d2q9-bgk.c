  ...
  */
  ...
  for (int ii = 0; ii < params.ny; ii++)
  {
    for (int jj = 0; jj < params.nx; jj++)
    {
      ...
      // scratch space grid
      tmp.speeds[0] = cellsIn[ii  * params.nx + jj ].speeds[0]; // central cell, no movement
      tmp.speeds[1] = cellsIn[ii  * params.nx + x_w].speeds[1]; // east
      tmp.speeds[2] = cellsIn[y_s * params.nx + jj ].speeds[2]; // north
      tmp.speeds[3] = cellsIn[ii  * params.nx + x_e].speeds[3]; // west
      tmp.speeds[4] = cellsIn[y_n * params.nx + jj ].speeds[4]; // south
      tmp.speeds[5] = cellsIn[y_s * params.nx + x_w].speeds[5]; // north-east
      tmp.speeds[6] = cellsIn[y_s * params.nx + x_e].speeds[6]; // north-west
      tmp.speeds[7] = cellsIn[y_n * params.nx + x_e].speeds[7]; // south-west
      tmp.speeds[8] = cellsIn[y_n * params.nx + x_w].speeds[8]; // south-east 
      ...
      { 
        ...
        for (int kk = 0; kk < NSPEEDS; kk++)
        {
          local_density += cellsOut[ii * params.nx + jj].speeds[kk];
        }

        // x-component of velocity
        u_x = (
                + cellsOut[ii * params.nx + jj].speeds[1]
                + cellsOut[ii * params.nx + jj].speeds[5]
                + cellsOut[ii * params.nx + jj].speeds[8]
                - cellsOut[ii * params.nx + jj].speeds[3]
                - cellsOut[ii * params.nx + jj].speeds[6]
                - cellsOut[ii * params.nx + jj].speeds[7]
              ) / local_density;
        // compute y velocity component
        u_y = (
                + cellsOut[ii * params.nx + jj].speeds[2]
                + cellsOut[ii * params.nx + jj].speeds[5]
                + cellsOut[ii * params.nx + jj].speeds[6]
                - cellsOut[ii * params.nx + jj].speeds[4]
                - cellsOut[ii * params.nx + jj].speeds[7]
                - cellsOut[ii * params.nx + jj].speeds[8]
              ) / local_density;
      ...
      // ===============
      // === REBOUND === if the cell contains an obstacle
      // ===============
			else
      {
        // called after propagate (streaming), so taking values from scratch space
        // mirroring, and writing into main grid
        cellsOut[ii * params.nx + jj].speeds[1] = tmp.speeds[3];
        cellsOut[ii * params.nx + jj].speeds[2] = tmp.speeds[4];
        cellsOut[ii * params.nx + jj].speeds[3] = tmp.speeds[1];
        cellsOut[ii * params.nx + jj].speeds[4] = tmp.speeds[2];
        cellsOut[ii * params.nx + jj].speeds[5] = tmp.speeds[7];
        cellsOut[ii * params.nx + jj].speeds[6] = tmp.speeds[8];
        cellsOut[ii * params.nx + jj].speeds[7] = tmp.speeds[5];
        cellsOut[ii * params.nx + jj].speeds[8] = tmp.speeds[6];
      }
    }
  }
